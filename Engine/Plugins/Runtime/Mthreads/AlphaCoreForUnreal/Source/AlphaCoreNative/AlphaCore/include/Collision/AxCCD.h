...
namespace AlphaCore
{
	namespace Collision
	{
			...
			bool* angleTestRtn = nullptr)
		{
			...
			if ((colliFreeWeight[0] < 0.0F - eps || colliFreeWeight[0]>1.0F + eps || 
				 colliFreeWeight[1] < 0.0F - eps || colliFreeWeight[1]>1.0F + eps ||
				-colliFreeWeight[2] < 0.0F - eps || -colliFreeWeight[2]>1.0F + eps || 
				-colliFreeWeight[3] < 0.0F - eps || -colliFreeWeight[3]>1.0F + eps))
				return false;
