...
#define AX_ERROR(...)	SPD_AUGMENTED_LOG(Error,(std::string(ERROR_HEAD)+__FUNCTION__+" | ").c_str(), __VA_ARGS__);
...
struct AxLogInfoBlock
{
	...
	ALPHA_SHARE_FUNC void LogInt2(const char* head, int val, int val2, const char* endMark = "\n")
	{
		char n[32];
		char n2[32];
		__itoa(val, n);
