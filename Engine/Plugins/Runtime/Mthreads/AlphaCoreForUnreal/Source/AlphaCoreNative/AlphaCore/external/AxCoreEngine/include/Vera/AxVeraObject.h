...
public:
	...
	void setSaveCachePath();

	void setdynamicColliderGeo(AxSimObject* hou_dynamicColliderGeo);
	void setdynamicAttachGeo(AxSimObject* hou_dynamicAttachGeo);
	//by_hy
...
protected:
	...
	VeraSolverParamter veraSolverParamLast;
	...
	//by_hy
	AxUInt32 m_useExe;
	AxSimObject* dynamicColliderGeo;
	AxSimObject* dynamicAttachGeo;
	//by_hy
...
private:
	...
	void HDKupdateSceneGeometry(AxFp32 dt);
	void HDKupdateSceneGeometryDevice(AxFp32 dt);
	void copyAllBuffer(const std::vector<std::string>& propList, AxSimObject* copyFrom, AxSimObject* copyTo, bool loadToDevice = false);
	...
	void _collisionResolveHistoryDataIO(bool savePoint2ContactData);

	AxUInt32 m_CollisionDebugFStreamingTimes;
	AxUInt32 m_CollisionHistoryDebugFStreamingTimes;

 
	void _constraintProjDebugIO(std::string stateName = "");//新增debug打印数据用

	AxUInt32 m_iConstraintDebugIOTimes;//新增debug打印数据用
};
