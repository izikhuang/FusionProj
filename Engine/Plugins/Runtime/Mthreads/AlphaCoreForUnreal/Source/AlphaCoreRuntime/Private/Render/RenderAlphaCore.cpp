...
DEFINE_LOG_CATEGORY_STATIC(AlphaCoreRender, Log, All);
...
namespace RenderAlphaCore
{
		...
		FGlobalIlluminationPluginResources& Resources)
    {
		...
		// check if AlphaCore AxSimWorld has SimObjects
		...
		(FRHICommandListImmediate& RHICmdList)
			{
				...
				/////////////////////////////////
				auto SceneManager = AlphaCoreSceneManager::GetInstance();
				if (SceneManager->GetSimWorldStatus() == 0) return; // world Not Inited
				
				AxSimWorld* world = SceneManager->GetWorld();
				if (!world) return;
				if (world->GetSimObjectNum() == 0) return;

				SceneManager->SetSimWorldRendering();

				// Create And Init RenderScene
				if (!world->HasSceneObject())
				{
				...
				FVector3f UECameraOrigin = ViewVoxelizeParameters.PrevRelativeWorldCameraOrigin;
				FVector3f UECameraUp = ViewVoxelizeParameters.ViewUp;
				FVector3f UECameraForward = ViewVoxelizeParameters.ViewForward;



				// Convert to AxParameter
				//FVector convertedLightDir = UELightDirection.RotateAngleAxis(-90, FVector::ZAxisVector).
				//	RotateAngleAxis(90, FVector::XAxisVector);
				//FVector trickLightPosition = convertedLightDir * -1000000.f;
				//FVector3f convertedCameraOrigin = UECameraOrigin.RotateAngleAxis(-90, FVector3f::ZAxisVector).
				//	RotateAngleAxis(90, FVector3f::XAxisVector) * -1.0;
				//FVector3f convertedCameraForward = UECameraForward.RotateAngleAxis(-90, FVector3f::ZAxisVector).
				//	RotateAngleAxis(90, FVector3f::XAxisVector) * -1.0f;
				//FVector3f convertedCameraUp = UECameraUp.RotateAngleAxis(-90, FVector3f::ZAxisVector).
				//	RotateAngleAxis(90, FVector3f::XAxisVector) * -1.0f;

				//AxVector3 LightPivot = { trickLightPosition.X,trickLightPosition.Y,trickLightPosition.Z };
				//AxVector3 CamOrigin = { convertedCameraOrigin.X,convertedCameraOrigin.Y,convertedCameraOrigin.Z };
				//AxVector3 CamForward = { convertedCameraForward.X,convertedCameraForward.Y,convertedCameraForward.Z };
				//AxVector3 CamUp = { convertedCameraUp.X,convertedCameraUp.Y,convertedCameraUp.Z };

				FVector trickLightPosition = UELightDirection * -10000000.f;
				AxVector3 LightPivot = AlphaCoreUtils::ConvertUnrealPositionToAlphaCore(trickLightPosition);
				AxVector3 CamOrigin = AlphaCoreUtils::ConvertUnrealPositionToAlphaCore(UECameraOrigin);
				AxVector3 CamForward = AlphaCoreUtils::ConvertFromUnrealToAlphaCoreWithoutScale(UECameraForward);
				AxVector3 CamUp = AlphaCoreUtils::ConvertFromUnrealToAlphaCoreWithoutScale(UECameraUp);

				//UE_LOG(AlphaCoreRender, Warning, TEXT("LightPivot : %f,%f,%f"), LightPivot.x, LightPivot.y, LightPivot.z);
				//UE_LOG(AlphaCoreRender, Warning, TEXT("CamOrigin :  %f,%f,%f"), CamOrigin.x, CamOrigin.y, CamOrigin.z);
				//UE_LOG(AlphaCoreRender, Warning, TEXT("CamForward : %f,%f,%f"), CamForward.x, CamForward.y, CamForward.z);
				//UE_LOG(AlphaCoreRender, Warning, TEXT("CamUp : %f,%f,%f"), CamUp.x, CamUp.y, CamUp.z);
				...
				//UE_LOG(AlphaCoreRender, Warning, TEXT("cameraOrigin : %s"), *(UECameraOrigin.ToString()));
				//UE_LOG(AlphaCoreRender, Warning, TEXT("ViewUp : %s"), *(UECameraUp.ToString()));
				//UE_LOG(AlphaCoreRender, Warning, TEXT("ViewForward : %s"), *(UECameraForward.ToString()));
				//UE_LOG(AlphaCoreRender, Warning, TEXT("LightDirection : %s"), *(UELightDirection.ToString()));
				...
				for (int r = 0; r < ImageHeight; ++r)
				{
					for (int c = 0; c < ImageWidth; ++c)
					{
						world->GetDepthImage()->SetValue(
							r * ImageWidth + c,float(WorldPosData[r * ImageWidth + c].A) / 100.f);
					}
				}
				...
				//AxStormSysObject* obj = (AxStormSysObject*)world->GetObjectByIndex(0);

				//auto Cloud = obj->stormSimData.mRCloud;
				////Cloud->LoadToHost();
				//if (Cloud) 
				//{
				//	obj->stormSimData.LoadToHost();
				//	//Cloud->LoadToHost();
				//	for (int i = 0; i < (int)Cloud->GetNumVoxels(); i++)
				//	{
				//		float value = Cloud->GetValue(i);
				//		if (std::abs(value) >= 0.0001f)
				//		{
				//			UE_LOG(AlphaCoreRender, Warning, TEXT("CloudValue %d:%f"), i, value);
				//			break;
				//		}
				//	}
				//}
				...
				//	for (int c = 0; c < ImageWidth; ++c)
				//	{
				//		OutputData[r * ImageWidth + c].R = float(WorldPosData[r * ImageWidth + c].R)/ float(WorldPosData[r * ImageWidth + c].A);
				//		OutputData[r * ImageWidth + c].G = float(WorldPosData[r * ImageWidth + c].G)/float(WorldPosData[r * ImageWidth + c].A);
				//		OutputData[r * ImageWidth + c].B = float(WorldPosData[r * ImageWidth + c].B)/float(WorldPosData[r * ImageWidth + c].A);
				//		OutputData[r * ImageWidth + c].A = float(WorldPosData[r * ImageWidth + c].A)/float(WorldPosData[r * ImageWidth + c].A);
				//	}
				//}


				SceneManager->SetSimWorldRenderFinished();
