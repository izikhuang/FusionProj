...
namespace AlphaCore
{
	namespace Math
	{
		...
		ALPHA_SHARE_FUNC AxFp32  SmoothStep(AxFp32 t1, AxFp32 t2, AxFp32 x) {
		...
		//TODO : Re:SELF
		ALPHA_SHARE_FUNC AxFp32 LerpRampFloat(AxFp32* list, AxFp32 pos, AxUInt32 size)
		{
			AxFp32 position = ClampF32(pos, 0.0f, 1.0f - 1e-6) * (size - 1);
			AxFp32 flr = floorf(position);
			AxFp32 ceil = flr + 1;
			AxFp32 v1 = list[(AxInt32)flr] * (ceil - position);
			AxFp32 v2 = list[(AxInt32)ceil] * (position - flr);
			return v1 + v2;
		}
