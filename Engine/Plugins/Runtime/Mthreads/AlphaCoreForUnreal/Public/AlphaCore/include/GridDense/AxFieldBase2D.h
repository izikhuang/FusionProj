...
public:
	...
	AxField2DBase(AxUInt32 nx, AxUInt32 ny, std::string name = AlphaProperty::DensityField)
	{
	...
	void Init(AxUInt32 nx, AxUInt32 ny)
	{
		SetPivot(MakeVector3(0,0,0));
		SetFieldResolution(nx,ny);
		//TODO:TODO  TEST
		SetFieldSize(MakeVector2(m_FieldInfo.VoxelSize.x* nx, m_FieldInfo.VoxelSize.y*ny));
	}
	...
	void SetFieldResolution(AxUInt32 nx, AxUInt32 ny)
	{
	...
	T* GetRawData() { return m_CellBuffer.Data(); };
	T* GetRawDataDevice()
	{
	...
	void SetToZero()
	{
		memset(m_CellBuffer.Data(), 0, m_CellBuffer.Size()*sizeof(T));
	}
#ifdef ALPHA_CUDA
	void SetToZeroDevice() {
		cudaMemset(m_CellBuffer.GetDataRawDevice(), 0, m_CellBuffer.Size() * sizeof(T));
	}
#endif
	...
	bool CopyCellDataBuffer(AxField2DBase<T>* field)
	{
		if (field->GetNumVoxels() != this->GetNumVoxels())
			return false;
		memcpy(m_CellBuffer.Data(), field->GetRawData(), m_CellBuffer.Size() * sizeof(T));
		return true;
	}
