...
def makeDefaultCode(item):
    ...
    if dataType == "Vector3FParam":
        ...
        return vec3Temp
    if dataType == "Vector4FParam":
        vec4Temp = "MakeVector4(#Uf,#Vf,#Wf,#Xf)"
        vec4Temp = vec4Temp.replace("#U",str(item["RawValue"][0]))
        vec4Temp = vec4Temp.replace("#V",str(item["RawValue"][1]))
        vec4Temp = vec4Temp.replace("#W",str(item["RawValue"][2]))
        vec4Temp = vec4Temp.replace("#X",str(item["RawValue"][3]))
        return vec4Temp
