...
public:
	...
	virtual void Clear() {};
	virtual void ResizeStorage(AxUInt64 size) {};
...
struct AxSPMDTick
{
	AlphaCore::AxSPMDBackend LockType;
	int* CurrPos;
	void* Locker;
	ALPHA_SHARE_FUNC int AddIndex(int num = 1)
	{
		...
		if (LockType == AlphaCore::AxSPMDBackend::CPUx86)
			*CurrPos = last + num;
#else
		if (LockType == AlphaCore::AxSPMDBackend::CUDA) {
...
static AxSPMDTick MakeSPMDTick(AlphaCore::AxSPMDBackend type)
{
	AxSPMDTick tick;
	tick.LockType = type;
	if (tick.LockType == AlphaCore::AxSPMDBackend::CPUx86)
		tick.CurrPos = new int;
#ifdef ALPHA_CUDA
	if (tick.LockType == AlphaCore::AxSPMDBackend::CUDA)
...
public:
	...
	virtual void ClearAndDestory()
	{
		...
		{
			AX_WARN("device storage release  --- ? --- {}", m_sName);
	...
	void CopyBufferRaw(AxStorage<T>* other)
	{
		if (other->Size() != this->Size())
			this->Resize(other->Size());
		std::memcpy(m_Data.data(), other->m_Data.data(), other->GetCapacity());
	}
	...
	virtual void PrintData(const char* head = " ", unsigned int start = 0, int end = -1)
	{
		unsigned int _end = end < 0 ? m_Data.size() : std::min((int)m_Data.size(), end);
	...
	void PrintDataABTest(AxStorage<T>* other, AxUInt32 start = 0, int end = -1)
	{
		AxUInt32 _end = end < 0 ? m_Data.size() : std::min((int)m_Data.size(), end);
		for (size_t i = start; i < _end; ++i)
	...
	inline T&	GetValue(AxUInt64 index) { return m_Data[index]; };
	...
	void PrintDataFStreaming(std::string path, const char* head = " ", unsigned int start = 0, int end = -1)
	{
		...
		if (!ofs)
			return;
		AxUInt32 _end = end < 0 ? m_Data.size() : end;
		AX_INFO("PrintBuffer As TXT {} Size:{}  Range:[{} - {}]\n", path.c_str(), m_Data.size(), start, _end);
...
public:
	...
	virtual bool DeviceMalloc(bool loadToDevice = true)
	{
		...
		auto cudaRet = cudaMalloc((void**)&m_DevicePtr, this->GetCapacity());
		...
		else
		{
 			AX_INFO("Malloc device memory succ use | {:03.2f} MB | {}",
				(float)GetCapacity() / 1024.0f / 1024.0f,m_sName.c_str());
		}
...
public:
	...
	virtual AxUInt64 GetIOCapacity()
	{
		...
		// m_iVecSize		 AxUInt32
		// m_iBufferSize	 AxUInt64
		return m_sName.size() + 1 + sizeof(AxUInt32) + sizeof(AxUInt32) + sizeof(AxUInt32) +
			sizeof(AxUInt64) + this->GetCapacity();
	};
};
