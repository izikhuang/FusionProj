	...
	namespace GridDense
	{
		namespace CUDA
		{
			namespace Kernel
			{
				...
				template<class T>
				ALPHA_SPMD_FUNC __global__ void advect_SemiLagrangian(
					...
					uInt64 numVoxels)
				{
					...
					AxUInt32 x = index % res.x;
					AxUInt32 y = (index % nvSlice) / res.x;
					AxUInt32 z = index / nvSlice;
				...
				__global__ void vorticityConfinement(float* velXRaw, float* velYRaw, float* velZRaw, float vorticityScale)
				{
					//vorticityScale
				...
				ALPHA_SPMD_FUNC __global__ void clampExtrema(
					float * fieldOldRaw,
					float * fieldNewRaw,
					float * vxRaw,
					float * vyRaw,
				...
				ALPHA_SPMD_FUNC __global__ void fieldMix(
					float* retRaw,
					float* aRaw,
					float coeffA,
					float* bRaw,
				...
				__global__ void pressureSolverGaussSeidel_BLCOK(
					float* pressOldRaw,
					float* pressNewRaw,
					float* divRaw,
					...
					uInt64 numVoxels)
				{
					...
					//load pressureOldData ToShareMemory
					...
					//solve red black grid
					cg::sync(cta);


					//write new pressure data ShareMemory to global
				...
				template<class T>
				__global__ void advect_MacCormack(T* src, T* dst, T* u, T* v, T* w, float dt)
				{
