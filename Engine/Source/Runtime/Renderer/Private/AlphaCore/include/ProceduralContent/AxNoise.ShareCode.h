#ifndef __AX_NOISE_SHARECODE_H__
#define __AX_NOISE_SHARECODE_H__

#include <AxMacro.h>
#include "AxNoise.DataType.h"

ALPHA_DEVICE_CONSTANT static const AxInt32 PERMSIZE = 8192;
ALPHA_DEVICE_CONSTANT static const AxInt32 PERMMASK = 8191;
ALPHA_DEVICE_CONSTANT static const AxInt32 RTABLE_SIZE = 8200;
ALPHA_DEVICE_CONSTANT static const AxFp32 FREQ_ADJUST = 1.25f;
...
namespace AlphaCore {
	namespace ProceduralContent {
		namespace ShareCode 
		{
			...
			ALPHA_SHARE_FUNC float pow4(float x)
			{
				x *= x;
				return x * x;
			}

			ALPHA_SHARE_FUNC float pow3(float x)
			{
				return x * x * x;
			}

			ALPHA_SHARE_FUNC float pow2(float x)
			{
				return x * x;
			}

			ALPHA_SHARE_FUNC void extrapolate3(const int *thePx, const int *thePy, const int *thePz,
				const float *theXT,
			...
			ALPHA_SHARE_FUNC void extrapolate4v(const int *thePx, const int *thePy, const int *thePz, const int *thePw,
				const float *theXT, const float *theYT, const float *theZT,
				int xsb, int ysb, int zsb, int wsb,
				float x, float y, float z, float w,
				AxVector3 *n, AxVector3 *dndx, AxVector3 *dndy, AxVector3 *dndz, AxVector3 *dndw)
			{
				...
				if (attn < 2.0f)
				{
					...
					AxVector3 rx = MakeVector3(theXT[index], theYT[index], theZT[index]);
					AxVector3 ry = MakeVector3(theXT[index + 1], theYT[index + 1], theZT[index + 1]);
					AxVector3 rz = MakeVector3(theXT[index + 2], theYT[index + 2], theZT[index + 2]);
					AxVector3 rw = MakeVector3(theXT[index + 3], theYT[index + 3], theZT[index + 3]);
					AxVector3 b = rx * x + ry * y + rz * z + rw * w;
					*n += t * b;
			...
			ALPHA_SHARE_FUNC void xnoise3d(const void *theXNoise, AxVector3 p, float *n, float *dndx, float *dndy, float *dndz)
			{
				...
				*n = *dndx = *dndy = *dndz = 0.0f;
				...
				if (inSum <= 1)   //We're inside the tetrahedron (3-Simplex) at (0,0,0)
				{
					...
					{
						AxUChar c = (bScore > aScore ? bPoint : aPoint); //Our other closest vertex is the closest out of a and b.
					...
					{
						AxUChar c = (AxUChar)(aPoint | bPoint); //Our two extra vertices are determined by the closest two.
				...
				else     //We're inside the octahedron (Rectified 3-Simplex) in between.
				{
					...
					if (aIsFurtherSide == bIsFurtherSide)
					{
						if (aIsFurtherSide)   //Both closest points on (1,1,1) side
						{
							...
							AxUChar c = (AxUChar)(aPoint & bPoint);
							if ((c & 0x01) != 0)
							{
								dx_ext1 = dx0 - 2 - 2 * SQUISH_CONSTANT_3D;
								dy_ext1 = dy0 - 2 * SQUISH_CONSTANT_3D;
			...
			ALPHA_SHARE_FUNC void xnoisev4d(const void *theXNoise, AxVector4 p,
				AxVector3 *n, AxVector3 *dndx, AxVector3 *dndy, AxVector3 *dndz, AxVector3 *dndw)
			{
				const int *thePx = (const int *)theXNoise;
				const int *thePy = thePx + PERMSIZE;
				...
				*dndw = MakeVector3();
				...
				if (inSum <= 1)   //We're inside the pentachoron (4-Simplex) at (0,0,0,0)
				{
					...
					{
						AxUChar c = (AxUChar)(aPoint | bPoint); //Our three extra vertices are determined by the closest two.
				...
				else if (inSum <= 2)     //We're inside the first dispentachoron (Rectified 4-Simplex)
				{
					...
					if (aIsBiggerSide == bIsBiggerSide)
					{
						...
						{
							AxUChar c1 = (AxUChar)(aPoint | bPoint);
							AxUChar c2 = (AxUChar)(aPoint & bPoint);
							if ((c1 & 0x01) == 0)
							{
								xsv_ext0 = xsb;
								xsv_ext1 = xsb - 1;
			...
			ALPHA_SHARE_FUNC AxVector3 CurlXNoise4D(const void *theXNoise, AxVector4 p)
			{
				AxVector3 n = MakeVector3();
				AxVector3 dndx = MakeVector3();
				AxVector3 dndy = MakeVector3();
				AxVector3 dndz = MakeVector3();
				AxVector3 dndw = MakeVector3();
			...
			namespace Internal
			{
				ALPHA_SHARE_FUNC AxVector3 CurlNoise4DVector(
				...
				{
					// 4D position
					AxVector4 P = MakeVector4(pos, time);
					AxVector3 v = MakeVector3();
					AxFp32* controlPropRaw = noiseParam.ControlProperty;
					AxFp32 cThreshold = noiseParam.Threshold;
					bool UseControlField = false;
					AxFp32 rough = noiseParam.BaseParam.Roughness;
					AxInt32 turb = noiseParam.BaseParam.Turbulence;
					if (controlVal > cThreshold || controlPropRaw == nullptr)
					{
						AxFp32 scale = 1.0f;
						if (UseControlField)
						{
						...
						P *= noiseParam.BaseParam.Frequency;
						P -= noiseParam.BaseParam.Offset;
						...
						///if (atten != 1)
						///	v = pow(v, atten);
						v *= noiseParam.BaseParam.Amplitude *1.0f;// (dt * 24);//TODO : 24????
					}
					return v;
				}
			}

			ALPHA_SHARE_FUNC void CurlNoise4DVector(AxUInt32 idx,
				AxCurlNoiseParam noiseParam,
				AxVector3* posRaw,
				AxVector3* outVecRaw,
				void* noiseRawData,
